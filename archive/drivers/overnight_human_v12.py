"""Autonomous cycle: v11 recipe + curriculum session + WHITE amplification -> v12.
...
Steps (logged to OVERNIGHT_REPORT.md):
  1. mate_demos_v6    300 games @400 sims (defer hook -> ScriptedMate Black)
  2. heuristic_v7     800 games @400 sims (curriculum, all plies)
  ...
  4. human_blackfocus 300 games ; v12_whitefocus 250 games (@400 sims)
  5. merge -> combined_v10 ; corpus audit ; process ; train v12
  6. D2 ; benchmark v12 (60 games @400, seed 20260704) vs the aggressive anchor

Run from the repo root:  py -3 -u overnight_human_v12.py
"""
import glob
import json
import os
...
DEMO_DIR = os.path.join("data", "raw", "mate_demos_v6")
HEUR_DIR = os.path.join("data", "raw", "heuristic_v7")
...
BF_STARTS = os.path.join("data", "start_fens", "human_bf_starts_v2.jsonl")
WF_STARTS = os.path.join("data", "start_fens", "human_wf_starts_v1.jsonl")
BF_DIR = os.path.join("data", "raw", "human_blackfocus_v12")
WF_DIR = os.path.join("data", "raw", "v12_whitefocus")
MERGED_DIR = os.path.join("data", "raw", "combined_v10")
PROCESSED_DIR = os.path.join("data", "processed", "combined_v10")
MODEL_DIR = os.path.join("models", "fresh_start_v12")
...
def main():
    with open(REPORT, "a", encoding="utf-8") as f:
        f.write(f"\n# Human curriculum v12 run (white+black amplification, x{HUMAN_DUP} "
    ...
    d2()
    run("benchmark-v12", ["src/benchmark.py", "--model", MODEL_PT,
                          "--games", "60", "--sims", "400",
                          "--seed", "20260704"])
