...
def native_move(state, history, eval_fn, channels, sims, batch):
    ...
    tree.run_batched_puct(sims, eval_fn, batch_size=batch, channels=channels,
                          allow_early_stop=True, root_noise=False)
    action, _probs, _value = tree.best_action(temperature=0.0)
    return action


def python_move(state, evaluator, sims, batch):
    search = MCTS(num_simulations=sims, eval_fn=evaluator, root_noise=False,
                  allow_early_stop=True, batch_size=batch)
    action, _probs, _value = search.get_best_action(state, temperature=0.0)
...
def play(start_fen, native_is_white, eval_fn, channels, evaluator, sims, batch):
    ...
    while not state.is_terminal():
        ...
        uci = (native_move(state, history, eval_fn, channels, sims, batch)
               if native_turn else python_move(state, evaluator, sims, batch))
        if uci is None:
            break
